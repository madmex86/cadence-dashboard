...
Deno.serve(async (req) => {
  ...
  try {
    const { name, email, unsubscribe_url } = await req.json();
    if (!email || !name) return new Response(JSON.stringify({ error: 'Missing name or email' }), { status: 400, headers: { ...cors, 'Content-Type': 'application/json' } });
...
<table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;background:#fff;border-radius:6px;overflow:hidden;font-family:Georgia,serif;box-shadow:0 2px 12px rgba(0,0,0,.07);">
  ...
  <tr><td style="padding:36px 40px;color:#333;">
    <h1 style="font-size:24px;font-weight:400;color:#1a1610;margin:0 0 16px;font-family:Georgia,serif;">Welcome to the fold, ${name}!</h1>
    <p style="font-size:15px;line-height:1.8;margin:0 0 16px;">You&rsquo;re now on the Cadence Creatures mailing list &mdash; first to know when new creatures are spotted, drops launch, and adventures unfold.</p>
    <p style="font-size:15px;line-height:1.8;margin:0 0 28px;">Keep your eyes on the field notes.</p>
    <a href="https://cadencecreatures.com" style="display:inline-block;background:#C9A84C;color:#1a1610;text-decoration:none;padding:12px 28px;border-radius:3px;font-size:12px;letter-spacing:.12em;text-transform:uppercase;font-family:sans-serif;font-weight:600;">Visit the Bestiary</a>
  </td></tr>
  ${FOOTER(unsub)}
</table></td></tr></table></body></html>`;
    const res = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${RESEND_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: FROM, to: [email], subject: 'Welcome to Cadence Creatures ✦', html, text: `Welcome to the fold, ${name}!\n\nYou're now on the Cadence Creatures mailing list.\n\nVisit: https://cadencecreatures.com\n\n---\nUnsubscribe: ${unsub}` }) });
